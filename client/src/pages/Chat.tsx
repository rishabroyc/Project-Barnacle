...
export function Chat() {
  ...
  useEffect(() => {
    ...
    // Rehydrate prior history from the server, falling back to the
    // persona greeting if there's no backend/history yet (see issue #8).
    let cancelled = false;
    fetch(`/api/sessions/${sessionId.current}/messages`)
      ...
      .catch(() => {
        if (!cancelled) setMessages([{ role: "assistant", content: BRO_GREETINGS[broType] }]);
      });
  ...
  const sendUserMessage = async (text: string) => {
    ...
    try {
      const result = await sendMessage({ sessionId: sessionId.current, broType, messages: updated });
